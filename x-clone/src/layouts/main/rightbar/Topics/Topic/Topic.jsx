...
function Topic({ item }) {
    return (
        ...
        >
            ...
            {item?.postCount && (
                <div className='text-[13px] leading-4 mt-1'>
                    {numberFormat(item.postCount)} posts
                </div>
            )}
            <Popover className="absolute top-1.5 right-2">
                <PopoverButton
                    className='outline-none w-[34.75px] h-[34.75px] rounded-full text-[color:var(--color-base-secondary)]  flex items-center justify-center hover:bg-[#1d9bf01a] transition-colors '
                >
                ...
                <PopoverPanel className="z-[1] grid w-[384px] rounded-xl absolute top-2 right-2 overflow-hidden  bg-[color:var(--background-primary)]  shadow-box text-[15px] leading-5 font-bold text-[color:var(--color-base)]">
                    <button type='button' className='hover:bg-white/[0.05] transition-colors text-left py-3 px-4 flex items-center gap-3'>
